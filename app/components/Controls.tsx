...
import CircularVisualizer from "./Visualizer";
...
export const Controls = ({
  ...
  const { startMicrophone, stopMicrophone, microphoneOpen,microphone } = useMicrophone();
  const { formRef, onKeyDown } = useSubmit()
  ...
  return (
    <form onSubmit={submitter} ref={formRef}>
      ...
      <div className="relative flex items-center justify-center mt-32 content-between ">
        {/* <div className="absolute w-full -top-[4.5rem] py-4 flex justify-between">
          <Settings />
          <Download messages={messages} />
        </div> */}<div style={{marginLeft:70}}>

                {microphone && <CircularVisualizer microphone={microphone} />}
        </div>

        <div 
        // className="flex bg-[#101014] rounded-full"
        >
        <span
            // className={`rounded-tl-[2rem] rounded-bl-[2rem] ps-0.5 py-0.5 ${
            //   microphoneOpen
          ...
          >
            <Tooltip showArrow content="Toggle microphone on/off.">
              <a
                href="#"
                onClick={(e: any) => microphoneToggle(e)}
                className={`  w-16 md:w-20 sm:w-24 py-2 md:py-4 px-2 h-full sm:px-8 font-bold  text-light-900 text-sm sm:text-base flex items-center justify-center group`}
              >
                {/* {microphoneOpen && (
                  <div className="w-auto items-center justify-center hidden sm:flex absolute shrink-0">
                    <MicrophoneIcon
                ...
                <div className="w-auto flex items-center justify-center shrink-0">
                  {/* <MicrophoneIcon micOpen={microphoneOpen} className="h-5 md:h-24   animate-ping-short" /> */}
                </div>
                {/* <span>
                {microphoneOpen ? (
                  <>Now listening...</>
